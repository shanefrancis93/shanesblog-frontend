...
export async function GET() {
  try {
    ...
    if (!fs.existsSync(poemsDirectory)) {
      ...
      return NextResponse.json({ error: 'Poems directory not found' }, { status: 404 });
    }
    ...
    const poems = filenames.map(filename => {
      try {
        ...
        return {
          title: data.title || baseFilename.replace(/-/g, ' '),
          content: content.trim(),
          illustration: data.illustration || `/images/image_unknown.webp`,
    ...
    poems.sort((a, b) => Number(new Date((b as any).date)) - Number(new Date((a as any).date)));
